...
def _q_t_ew(molecule, temp):
    """
    Translational parition function using an exponential well approximation
    ...
    return 4.0 * np.pi * np.exp(beta * k) * cap_lambda * integral
...
def calc_entropy_ew(molecule, temp):
    ...
    return Constants.r * (1.5 - k * beta + np.log(q_t) + term_4)
...
class Molecule:
    ...
    def calculate_thermochemistry(self, temp=298.15, ss='1M', method='grimme',
        ...
        self.s_rot = calc_s_rot_rr(self, temp=temp)
    ...
    def __init__(self, filename, is_ts=False, real_freqs=True,
        ...
        self.ew_k_kcal = k_kcal
        self.ew_a_inv_ang = a_inv_ang
